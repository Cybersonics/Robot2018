...
public class TeleopCubeHandler extends Command {
	...
	protected void execute() {
		...
		if (dpad != -1) {
			cubeHandler.outtake(dpad, modulate(OUTTAKE_SPEED));
		} else {
			...
			if (rightMagnitude > STICK_DEADZONE) {
				...
				cubeHandler.intake((rightAngle + 180.0) % 360.0, modulate(rightMagnitude));
			} else if (leftMagnitude > STICK_DEADZONE) {
				cubeHandler.outtake(leftAngle, modulate(leftMagnitude));
			} else {
				cubeHandler.hold();
			}
		}
