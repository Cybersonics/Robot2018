...
public class Drive extends Subsystem {
	...
	public void swerveDrive(double strafe, double forward, double omega, boolean useDeadzone, boolean usePID) {
		...
		double leftRearDelta = leftRearAngle - leftRearPosition;
		if (Math.abs(leftRearDelta) > 180.0) {
			leftRearDelta -= 360.0 * Math.signum(leftRearDelta);
		}
		if (Math.abs(leftRearDelta) > 90.0) {
		...
		double rightFrontDelta = rightFrontAngle - rightFrontPosition;
		if (Math.abs(rightFrontDelta) > 180.0) {
			rightFrontDelta -= 360.0 * Math.signum(rightFrontDelta);
		}
		if (Math.abs(rightFrontDelta) > 90.0) {
		...
		double rightRearDelta = rightRearAngle - rightRearPosition;
		if (Math.abs(rightRearDelta) > 180.0) {
			rightRearDelta -= 360.0 * Math.signum(rightRearDelta);
		}
		if (Math.abs(rightRearDelta) > 90.0) {
		...
		if (usePID) {
			RobotMap.driveLeftFront.set(ControlMode.Velocity, leftFrontSpeed * 700.0 / WHEEL_SPEED_MAXIMUM);
			RobotMap.driveLeftRear.set(ControlMode.Velocity, leftRearSpeed * 700.0 / WHEEL_SPEED_MAXIMUM);
			RobotMap.driveRightFront.set(ControlMode.Velocity, rightFrontSpeed * 700.0 / WHEEL_SPEED_MAXIMUM);
			RobotMap.driveRightRear.set(ControlMode.Velocity, rightRearSpeed * 700.0 / WHEEL_SPEED_MAXIMUM);
		} else {
