...
public class UltrasonicPositioning {
	...
	public static final double FRONT_X = 12.875, FRONT_Y = 12.0, SIDE_X = 15.125, SIDE_Y = 9.75;
	...
	private void update() {
		for (int i = 0; i < ULTRASONIC_COUNT; i++) {
			...
			double period;
			if (echo.get() >= 2 && Double.isFinite(period = echo.getPeriod()) && period < MAX_PERIOD) {
		...
		String correctionList = "", separator = "";
		for (UltrasonicTarget target : TARGETS) {
			if (target.availableArea.contains(position)) {
				double wallNormalAngle = Math.toDegrees(Math.atan2(target.wall.y2 - target.wall.y1, target.wall.x1 - target.wall.x2));
				double robotToWallNormalAngle = ((wallNormalAngle - heading) % 360.0 + 360.0) % 360.0;
				if (robotToWallNormalAngle % 90.0 < MAX_ANGLE_TO_WALL || robotToWallNormalAngle % 90.0 > 90.0 - MAX_ANGLE_TO_WALL) {
					...
					if (rangesValid[side.sensorA] && rangesValid[side.sensorB]) {
						...
						if (rangeDeviation < MAX_RANGE_DEVIATION && rangeDifferenceDeviation < MAX_RANGE_DIFFERENCE_DEVIATION) {
							...
							correctionList += separator + target.label;
							separator = ", ";
						}
					}
				}
		...
		SmartDashboard.putString("UltrasonicCorrection", correctionList);
	...
	private static enum RobotSide {
		FRONT(5, 0, new Point2D.Double(-FRONT_X, FRONT_Y), new Point2D.Double(FRONT_X, FRONT_Y)),
		RIGHT(1, 2, new Point2D.Double(SIDE_X, SIDE_Y), new Point2D.Double(SIDE_X, -SIDE_Y)),
		REAR(3, 6, new Point2D.Double(FRONT_X, -FRONT_Y), new Point2D.Double(-FRONT_X, -FRONT_Y)),
		LEFT(7, 4, new Point2D.Double(-SIDE_X, -SIDE_Y), new Point2D.Double(-SIDE_X, SIDE_Y));
		
		public final int sensorA, sensorB;
		public final Point2D.Double sensorAPosition, sensorBPosition;
		
		private RobotSide(int sensorA, int sensorB, Point2D.Double sensorAPosition, Point2D.Double sensorBPosition) {
